...
class MaxValueWidget(QFrame):
    ...
    def init_ui(self):
        ...
        self.max_value_label.setFont(QFont("Arial", 36, QFont.Bold))
...
class MaxWaveformWidget(QFrame):
    """最高値波形表示ウィジェット"""
    ...
    def init_ui(self):
        ...
        # タイトル
        # title = QLabel("最高値の波形")
        title = QLabel("最高値の波形 / Max Value Waveform")
        title.setFont(QFont("Arial", 24, QFont.Bold))
        title.setAlignment(Qt.AlignCenter)
        ...
        self.max_line = self.plot_widget.plot(
            [], [], pen=pg.mkPen("r", width=4), name="最高値の波形"
        )
        self.bolt_line = self.plot_widget.plot(
            [], [], pen=pg.mkPen("g", width=3), name="ウサイン・ボルト"
        )
    ...
    def update_waveform(self, mountain_data):
        """波形を更新"""
        if mountain_data:
            times = [data[0] for data in mountain_data]
            forces = [data[1] for data in mountain_data]

            # 時間を0からの相対時間に正規化
            base_time = times[0] if times else 0
            normalized_times = [t - base_time for t in times]

            self.max_line.setData(normalized_times, forces)

            # グラフの範囲調整
            self.adjust_plot_range(normalized_times, forces)

    def adjust_plot_range(self, current_times, current_forces):
        """グラフの表示範囲を調整"""
        all_times = current_times.copy()
        all_forces = current_forces.copy()
...
class RealtimeDisplayWidget(QFrame):
    ...
    def detect_mountain(self, timestamp, force):
        ...
        # 山の終了検出: 正の値 → 0
            ...
            if current_time - self.last_mountain_time >= 10.0:
                # 山を検出した（10秒経過している場合のみ）
                self.mountain_count += 1
                self.status_label.setText(f"山#{self.mountain_count}を検出！")
                ...
                # 10秒以内の場合は無視
                remaining_time = 10.0 - (current_time - self.last_mountain_time)
    ...
    def update_plot(self):
        ...
        # 現在位置の縦線を中央（最新時刻から2.5秒前）に配置
        current_position = latest_time - 2.5
        self.current_position_line.setPos(current_position)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 上右：最高値波形表示ウィジェット
        self.max_waveform_widget = MaxWaveformWidget()
        upper_layout.addWidget(self.max_waveform_widget, 2)  # 比率2
    ...
    def on_mountain_detected(self, mountain_data):
        """山が検出された時の処理"""
        # 最高値波形ウィジェットに波形データを送信
        self.max_waveform_widget.update_waveform(mountain_data)

    def on_max_value_updated(self, timestamp, force, mountain_data):
        """最高値が更新された時の処理"""
        # 最高値ウィジェットを更新し、最高値の波形データを取得
        max_mountain_data = self.max_value_widget.update_max_value(
            timestamp, force, mountain_data
        )

        # 最高値が更新された場合は波形も更新
        if mountain_data and len(mountain_data) > 0:
            self.max_waveform_widget.update_waveform(mountain_data)
